...
const TableFooter = () => {
    ...
    async function fetchPlayers(page) {
        const response = await fetch(`http://localhost:3001/players?size=${PAGE_SIZE}&from=${page}`, {
          headers: {
    ...
    return (
        <div className="table__footer">
            ...
            <span>{`Page ${Math.floor(pageData.from/PAGE_SIZE) + 1} out of ${Math.ceil(pageData.total/PAGE_SIZE)}`}</span>
